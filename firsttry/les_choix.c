...
extern int id; // Declare id variable from main.c
...
int find(int sid) {
    int found = 0; // Variable to track if any matching elements are found
...
void modify(){
    ...
    printf("what do you wanna modify\n");
    int mchoix ;scanf("%d",&mchoix);
    switch(mchoix){
        case 1:
            printf("enter new des...\n");scanf(" %[^\n]", elements[i].description);
        break;
        case 2:
        ...
        case 3:
            ...
            printf("enter new deadline\n");scanf(" %99[^,]",elements[i].deadline);
        break;
        default:
            printf("sorry but we dont have that method yet\n");
    }
    int lenght = calcLength();
    printf("%d",lenght);
    save(lenght);
}
int calcLength() {
